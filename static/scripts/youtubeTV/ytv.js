  ...
  var YTV =
    ...
    function (id, opts) {
      var noop = function () {},
        settings = {
          ...
          events: {
            ...
            stateChange: () => {
              document.querySelector(".ytv-video").style.opacity = 1;
            },
          },
        },
        cache = {
                  ...
                  handle.onload = function () {
                    ...
                    if (
                      Object.prototype.hasOwnProperty.call(
                        JSON.parse(handle.responseText),
                        "error"
                      )
                    ) {
                      cache.remove(url);
                      ...
                      console.log(
                        "Youtube-TV Error: Youtube API Response: " +
                          e.error.errors[0].reason +
                          "\n" +
                          "Details: " +
          ...
          endpoints: {
            ...
            playlistVids: function () {
              return (
                ...
                "&key=" +
                apiKey +
                "&maxResults=50&part=contentDetails"
              );
            },
            videoInfo: function () {
              return (
                ...
                "&key=" +
                apiKey +
                "&maxResults=50&part=snippet,contentDetails,status,statistics"
              );
            },
          },
        ...
        prepare = {
          ...
          build: function () {
            ...
            utils.addCSS(
              "#" +
                id +
                " .ytv-list .ytv-active a{border-left-color: " +
                settings.accent +
                ";}"
            );
            // Responsive CSS
          ...
          userUploads: function (userInfo) {
            if (userInfo && userInfo.items.length > 0) {
              settings.pid =
                userInfo.items[0].contentDetails.relatedPlaylists.uploads;
              utils.ajax.get(
                utils.endpoints.playlistVids(),
                prepare.compileVideos
              );
            } else
              console.log(
                "Youtube-TV Error: API returned no matches for: " +
                  (settings.channelId ? settings.channelId : settings.user) +
            ...
            if (playlistInfo && playlistInfo.items.length > 0) {
              if (!settings.channelId && !settings.user) {
                settings.cid =
                  "id=" +
                  (settings.channelId =
                    playlistInfo.items[0].snippet.channelId);
              }
              ...
              console.log(
                "Youtube-TV Error: API returned no matches for playlist(s): " +
                  settings.playlist
              );
          },
          ...
          playlists: function (res) {
            if (res && res.items.length > 0) {
              ...
              for (i = 0; i < playlists.length; i++) {
                ...
                list +=
                  '<a href="https://www.youtube.com/channel/UCjZKpZnWt6uKowt7efFO1iQ/" data-ytv-playlist="' +
                  data.plid +
                  '">'; //was list += '<a href="#" data-ytv-playlist="'+(data.plid)+'">';
                list +=
                  '<div class="ytv-thumb"><div class="ytv-thumb-stroke"></div><img src="' +
                  data.thumb +
                  '"></div>';
              ...
              var lh = settings.element.getElementsByClassName(
                  "ytv-list-header"
                )[0],
                headerLink = lh.children[0];
              headerLink.href =
                "https://www.youtube.com/channel/UCjZKpZnWt6uKowt7efFO1iQ/"; //was #
              headerLink.target = "_blank"; //was ''
              headerLink.setAttribute("data-ytv-playlist-toggle", ""); //Was true
              settings.element.getElementsByClassName(
                "ytv-list-header"
              )[0].innerHTML += list;
            ...
            if (data && data.items.length > 0) {
              utils.ajax.get(utils.endpoints.userInfo(), function (userInfo) {
                ...
                for (i = 0; i < videos.length; i++) {
                  if (videos[i].status.embeddable) {
                    ...
                    list +=
                      ...
                      '" class="ytv-clear">';
                    list +=
                      '<div class="ytv-thumb"><div class="ytv-thumb-stroke"></div><span>' +
                      time +
                      '</span><img src="' +
                      video.thumb +
                      '"></div>';
                    list +=
                      '<div class="ytv-content"><b>' + video.title + "</b>";
                    if (video.stats) {
                      list +=
                        '</b><span class="ytv-views">' +
                        utils.addCommas(video.stats.viewCount) +
                        " Views</span>";
                    }
                    list += "</div></a></li>";
                  }
                ...
                settings.element.innerHTML =
                  '<div class="ytv-relative"><div class="ytv-video"><div id="ytv-video-player"></div></div><div class="ytv-list">' +
                  list +
                  "</div></div>";
                if (
                  settings.element.getElementsByClassName("ytv-active")
                    .length === 0
                ) {
                  settings.element.getElementsByTagName("li")[0].className =
                    "ytv-active";
                }
                var active = settings.element.getElementsByClassName(
                  "ytv-active"
                )[0];
                ...
                if (settings.playlist) {
                  utils.ajax.get(
                    utils.endpoints.playlistInfo(settings.playlist),
                    prepare.playlists
                  );
                } else if (settings.browsePlaylists) {
                  utils.ajax.get(
                    utils.endpoints.userPlaylists(),
                    prepare.playlists
                  );
                }
              });
        ...
        action = {
          logic: {
            ...
            loadVideo: function (slug, autoplay) {
              var house = settings.element.getElementsByClassName(
                "ytv-video"
              )[0];
              var counter = settings.element.getElementsByClassName(
                "ytv-video-playerContainer"
              ).length;
              house.innerHTML =
                '<div id="ytv-video-player' +
                id +
                counter +
                '" class="ytv-video-playerContainer"></div>';
              ...
              cache.player = new YT.Player("ytv-video-player" + id + counter, {
                ...
                events: {
                  ...
                  onStateChange: function (e) {
                    if (
                      e.target.getPlayerState() === 0 &&
                      settings.chainVideos
                    ) {
                      var ns = settings.element.getElementsByClassName(
                          "ytv-active"
                        )[0].nextSibling,
                        link = ns.children[0];
                      link.click();
          ...
          endpoints: {
              ...
              setTimeout(() => {
                ...
                if (target) {
                  ...
                  if (target.getAttribute("data-ytv")) {
                    ...
                    var activeEls = settings.element.getElementsByClassName(
                        "ytv-active"
                      ),
                      i;
                    ...
                    action.logic.loadVideo(
                      target.getAttribute("data-ytv"),
                      true
                    );
                  }
                }
            ...
            playlistClick: function (e) {
              ...
              if (target && target.getAttribute("data-ytv-playlist")) {
                ...
                utils.ajax.get(
                  utils.endpoints.playlistInfo(settings.pid),
                  function (res) {
                    var lh = settings.element.getElementsByClassName(
                      "ytv-list-header"
                    )[0];
                    lh.className = lh.className.replace(
                      " ytv-playlist-open",
                      ""
                    );
          ...
          bindEvents: function () {
            utils.events.addEvent(
              settings.element,
              "click",
              action.endpoints.videoClick
            );
            utils.events.addEvent(
              settings.element,
              "click",
              action.endpoints.playlistToggle
            );
            utils.events.addEvent(
              settings.element,
              "click",
              action.endpoints.playlistClick
            );
          },
        },
        initialize = function (id, opts) {
          ...
          settings.element =
            typeof id === "string" ? doc.getElementById(id) : id;
          if (
            settings.element &&
            (settings.user || settings.channelId || settings.playlist)
          ) {
            prepare.youtube();
            prepare.build();
            action.bindEvents();
            ...
            console.log(
              "Youtube-TV Error: Missing either user, channelId, or playlist"
            );
        };
      ...
      this.fullscreen = {
        ...
        exit: function () {
          if (settings.element.className.indexOf("ytv-full") !== -1) {
            settings.element.className = settings.element.className.replace(
              "ytv-full",
              ""
            );
          }
